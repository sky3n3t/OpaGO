...
func newVM(opts vmOpts, runtime *wazero.Runtime) (*VM, error) {
	...
	if opts.parsedData != nil {
		if uint32(vm.module.env.Memory().Size(vm.ctx)-uint32(vm.baseHeapPtr)) < uint32(len(opts.parsedData)) {
			delta := uint32(len(opts.parsedData)) - (uint32(vm.module.env.Memory().Size(vm.ctx)) - uint32(vm.baseHeapPtr))
			_, success := vm.module.env.Memory().Grow(vm.ctx, Pages(delta))
			if !success {
				return nil, errors.New("exceeds max data")
			}
		}
		vm.module.env.Memory().Write(vm.ctx, uint32(vm.baseHeapPtr), opts.parsedData)
...
func (i *VM) SetPolicyData(ctx context.Context, opts vmOpts) error {
	...
	if opts.parsedData != nil {
		...
		i.module.writeMemPlus(uint32(i.baseHeapPtr), opts.parsedData)
...
func (i *VM) Eval(ctx context.Context,
	...
	if input != nil {
		...
		err := i.module.writeMemPlus(uint32(i.evalHeapPtr), raw)
		if err != nil {
			return nil, err
		}
